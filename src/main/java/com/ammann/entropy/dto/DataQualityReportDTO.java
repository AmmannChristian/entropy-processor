...
public record DataQualityReportDTO(
        @Schema(description = "Total number of events analyzed") Long totalEvents,
        @Schema(description = "List of sequence gaps represented as ranges (memory-efficient)")
                List<SequenceGapDTO> sequenceGaps,
        @Schema(description = "Number of distinct sequence gaps") Integer gapCount,
        @Schema(description = "Count of missing sequences (total across all gaps)")
                Long missingSequenceCount,
        @Schema(
                        description =
                                "Deprecated: Always empty. Use sequenceGaps for gap information.",
                        deprecated = true)
                List<Long> missingSequences,
        @Schema(
                        description =
                                "Clock drift analysis between edge ingestion time and server"
                                        + " reception time")
                ClockDriftInfoDTO clockDrift,
        @Schema(description = "Entropy decay rate plausibility check") DecayRateInfoDTO decayRate,
        @Schema(description = "Average network delay in milliseconds") Double averageNetworkDelayMs,
        @Schema(description = "Overall quality score (0.0 - 1.0)") Double qualityScore,
        @Schema(description = "Timestamp of assessment") Instant assessmentTimestamp) {}
