...
/**
 * Health check for NIST SP 800-22 statistical test suite service.
 *
 ...
 * <p>Configuration:
 * <ul>
 *   <li>{@code nist.sp80022.health.required} - Make service mandatory for health (default: false)</li>
 *   <li>{@code nist.sp80022.health.timeout} - Health check timeout (default: 5s)</li>
 * </ul>
 *
 * @see <a href="https://csrc.nist.gov/publications/detail/sp/800-22/rev-1a/final">NIST SP 800-22</a>
 * @see <a href="https://github.com/grpc/grpc/blob/master/doc/health-checking.md">gRPC Health Checking</a>
 */
@Readiness
@ApplicationScoped
public class Nist22ServiceHealthCheck implements HealthCheck {
    private static final String HEALTH_CHECK_NAME = "nist-sp800-22-service";
    private static final String SERVICE_TYPE = "NIST-SP-800-22";

    @GrpcClient("sp80022-health-service")
    MutinyHealthGrpc.MutinyHealthStub healthClient;

    @ConfigProperty(name = "nist.sp80022.health.required", defaultValue = "false")
    boolean required;

    @ConfigProperty(name = "nist.sp80022.health.timeout", defaultValue = "5s")
    Duration timeout;

    @ConfigProperty(
            name = "quarkus.grpc.clients.sp80022-test-service.host",
            defaultValue = "nist-sp800-22")
    String host;

    @ConfigProperty(name = "quarkus.grpc.clients.sp80022-test-service.port", defaultValue = "50051")
    int port;
    ...
    public HealthCheckResponse call() {
        ...
        if (healthClient == null) {
            Log.warn("NIST SP 800-22 gRPC health client not injected");
        ...
        try {
            ...
            if (serving) {
                Log.debug("NIST SP 800-22 service health check passed");
                return buildResponse(responseBuilder, true, startTime, null);
            } else {
                String status = grpcResponse.getStatus().name();
                Log.warnf("NIST SP 800-22 service not serving: %s", status);
                return buildResponse(
            ...
            if (code == Status.Code.UNAVAILABLE) {
                Log.warnf("NIST SP 800-22 service unreachable: %s", e.getStatus());
            } else {
                Log.warnf("NIST SP 800-22 service error: %s", e.getStatus());
            }
            ...
            if (isTimeoutException(e)) {
                Log.warn("NIST SP 800-22 service health check timeout");
                return buildResponse(
                        responseBuilder,
                        false,
                        startTime,
            ...
            Log.error("Unexpected error in NIST SP 800-22 health check", e);
            return buildResponse(
