...
public class EntropyStatisticsService {
    ...
    // Maximum samples for quadratic-time algorithms (Sample Entropy, Approximate Entropy)
     ...
     *
     * <p>Computes entropy from histogram probabilities using the standard base-2 definition.
     *
     ...
     *
     * <p>Computes Renyi entropy for the specified order parameter alpha.
     *
     ...
     * <p>
     * Note: This algorithm has quadratic complexity. For large datasets (over 2000 samples),
    ...
    public double calculateSampleEntropy(List<Long> intervalsNs, int m, double r) {
        ...
        // Downsample if dataset is too large for quadratic-time processing.
        List<Long> processedData =
                downsampleIfNeeded(intervalsNs, MAX_SAMPLES_FOR_QUADRATIC_ALGORITHMS);
        ...
        for (int i = 0; i < n - m; i++) {
            ...
            List<Double> templateMPlus1 = (i + m < n) ? normalizedData.subList(i, i + m + 1) : null;
        ...
        LOG.debugf(
                "Sample entropy (m=%d, r=%.3f) calculated: %.4f from %d intervals (processed: %d)",
    ...
    public double calculateApproximateEntropy(List<Long> intervalsNs, int m, double r) {
        ...
        LOG.debugf(
                "Approximate entropy (m=%d, r=%.3f) calculated: %.4f from %d intervals (processed:"
                        + " %d)",
    ...
    public EntropyAnalysisResult calculateAllEntropies(List<Long> intervalsNs, int bucketSizeNs) {
        ...
        LOG.infof(
                "Entropy analysis completed in %.2fms: Shannon=%.3f, Renyi=%.3f, Sample=%.3f,"
                        + " ApEn=%.3f",
                processingTimeNanos / 1_000_000.0,
                shannon,
                renyi,
                sampleEntropy,
                approximateEntropy);
    ...
    private void validateInput(List<Long> intervals, String algorithmName) {
        ...
        if (intervals.size() < MIN_SAMPLES_FOR_RELIABLE_ENTROPY) {
            LOG.warnf(
                    "%s: Only %d samples available, minimum %d recommended for reliable results",
                    algorithmName, intervals.size(), MIN_SAMPLES_FOR_RELIABLE_ENTROPY);
        }

        // Check for invalid values
        long invalidCount =
                intervals.stream().mapToLong(Long::longValue).filter(x -> x <= 0).count();
        if (invalidCount > 0) {
            throw new IllegalArgumentException(
                    String.format(
                            "Found %d invalid (non-positive) intervals in dataset", invalidCount));
        }
    }
    ...
    private List<Double> normalizeToUnitRange(List<Long> data) {
        ...
        return data.stream().map(val -> (double) (val - min) / range).toList();
    }

    /** Returns {@code true} if every element-wise difference is within the given tolerance. */
    private boolean templatesMatch(
            List<Double> template1, List<Double> template2, double tolerance) {
    ...
    private BasicStatistics calculateBasicStatistics(List<Long> data) {
        ...
        double variance =
                data.stream().mapToDouble(x -> Math.pow(x - mean, 2)).average().orElse(0.0);
    ...
    private List<Long> downsampleIfNeeded(List<Long> data, int maxSamples) {
        ...
        LOG.infof(
                "Downsampling %d samples to %d (step=%d) to prevent timeout",
