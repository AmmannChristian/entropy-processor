...
public class PublicEventsResource {
    ...
    @Operation(
            ...
            description = "Returns minimal public event activity. Limited to 10 events and safe fields only."
    )
    @APIResponses({
            @APIResponse(responseCode = "200", description = "Recent activity retrieved successfully",
                    content = @Content(schema = @Schema(implementation = PublicActivityResponseDTO.class))),
    ...
    public Response getRecentActivity(
        ...
        List<PublicEventSummaryDTO> publicEvents = events.stream()
                .sorted((a, b) -> Long.compare(a.hwTimestampNs, b.hwTimestampNs))
                .map(event -> new PublicEventSummaryDTO(
                        event.id,
        ...
        Instant latestActivity = events.stream()
                .map(event -> event.serverReceived)
                .max(Instant::compareTo)
                .orElse(null);

        PublicActivityResponseDTO response = new PublicActivityResponseDTO(
                publicEvents,
                publicEvents.size(),
                latestActivity
        );

        LOG.infof("Returned %d public activity events (latest: %s)", publicEvents.size(), latestActivity);
